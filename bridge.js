  ...
  sock.ev.on('messages.upsert', async ({ messages }) => {
    try { 
      ...
      if (!hasPushName && history.length <= 1) {
        ...
        // تطبيق التأخير المظبوط للعملاء الجدد مع الـ composing
      ...
      // التعديل السحري للتأخير والإشعارات: تفعيل الـ composing لمدة 3 ثوانٍ فقط، ثم الإرسال الفوري لضمان جرس الإشعار
      await sock.sendPresenceUpdate('composing', sendTo);
      await new Promise(r => setTimeout(r, 3000));
